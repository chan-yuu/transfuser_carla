...
class LidarCenterNet(nn.Module):
    ...
    def visualize_model_io(self, save_path, step, config, rgb, lidar_bev, target_point,
        ...
        if config.multitask:
            ...
            # 修复：保持原始宽高比，resize到合适尺寸
            # 原始: 高320, 宽704 → resize到高度306，保持比例
            target_height = 306
            target_width = int(ds_image.shape[1] * target_height / ds_image.shape[0])
            ds_image = cv2.resize(ds_image, (target_width, target_height))
        ...
        # BEV保持原始尺寸，添加文字区域
        bev_image = np.concatenate([bev_image, np.zeros((50, bev_image.shape[1], 3), dtype=np.uint8)], axis=0)
        ...
        rgb_image = cv2.resize(rgb_image, (1408, 352))

        # 固定下方三个图像的宽度，总宽度等于rgb宽度1408
        # BEV: 256宽
        # LiDAR: 512宽
        # 深度+语义: 640宽
        # 总计: 256 + 512 + 640 = 1408
        target_height = 306
        bev_image_resized = cv2.resize(bev_image, (256, target_height))
        images_resized = cv2.resize(images, (512, target_height))
        ds_image_resized = cv2.resize(ds_image, (640, target_height))

        assert (config.multitask)
        images = np.concatenate((bev_image_resized, images_resized, ds_image_resized), axis=1)
