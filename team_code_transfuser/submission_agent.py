...
class HybridAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file, route_index=None):
        ...
        if ('debug' in self.args):
            self.config.debug = bool(self.args['debug'])
        elif SAVE_PATH is not None:
            # 如果设置了SAVE_PATH但没有debug参数，自动启用debug
            self.config.debug = True
            print(f"[Visualize] SAVE_PATH detected, auto-enabling debug mode for visualization")
    ...
    def run_step(self, input_data, timestamp):
        ...
        with torch.no_grad():
            ...
            for i in range(self.model_count):
                ...
                model_end = time.time()
                single_model_times.append((model_end - model_start) * 1000)  # ms
        ...
        avg_single_ms = sum(single_model_times) / len(single_model_times)
        total_hz = 1000.0 / total_inference_ms if total_inference_ms > 0 else 0

        # 打印推理延迟信息
        if self.model_count > 1:
            print(f'[Inference] Total: {total_inference_ms:.1f}ms ({total_hz:.1f}Hz) | '
                  f'Model count: {self.model_count} | Avg single: {avg_single_ms:.1f}ms | '
                  f'Single times: {[f"{t:.1f}ms" for t in single_model_times]}')
        else:
            print(f'[Inference] Total: {total_inference_ms:.1f}ms ({total_hz:.1f}Hz)')
